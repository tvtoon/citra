...
*/
   case SDL_KEYDOWN:
// if (state == SDL_PRESSED)
    InputCommon::GetKeyboard()->PressKey(event.key.keysym.scancode);
    break;
...
{
//Core::System::SendSignal(Core::System::Signal::Shutdown);
...
{
//Core::System::SendSignal(Core::System::Signal::Reset);
//Core::System::GetInstance().RequestReset();
  std::cout << "Emulator Reset is broken, bribe your maintainer to fix it!" << std::endl;
}
    else
{
     InputCommon::GetKeyboard()->ReleaseKey(event.key.keysym.scancode);
}
